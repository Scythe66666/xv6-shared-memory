...
int main(){
    ...
    shm_id1 = shmget(IPC_PRIVATE, 10000, 0664);  // note only read permission is given
    if(shm_id1 < 0){
    ...
    shm_id2 = shmget(IPC_PRIVATE, 10000, 0666);  // note only read permission is given
    if(shm_id2 < 0){
        print_shm_error(shm_id2);
        exit();
    }
    printf(0, "\nNOTE: Done getting SECOND segment since its required for testing ahead.\n");
    ...
    shmctl(shm_id2, IPC_SET, &buf);


    shmctl(shm_id2, IPC_STAT, &buf);    //since we cant directly acces shms in userland code we get it via shmctl
